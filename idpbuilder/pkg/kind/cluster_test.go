...
func registryCertsHostPathPattern() string {
	tempDir := filepath.Clean(os.TempDir())
	tempDir = strings.ReplaceAll(tempDir, `\\`, `/`)
	return regexp.QuoteMeta(tempDir) + `/idpbuilder-registry-certs\.d-[^\n]+`
}

func TestGetConfig(t *testing.T) {
	registryPathPattern := registryCertsHostPathPattern()
...
\s+hostPath: ` + registryPathPattern + `
